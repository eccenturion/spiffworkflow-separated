...
def create_app() -> FlaskApp:
    ...
    # Solo registrar user blueprint si están habilitados los endpoints API
    if app.config.get("SPIFFWORKFLOW_BACKEND_RUN_API_ENDPOINTS", True):
    ...
    # Solo iniciar scheduler si el background processing está habilitado
    if not app.config.get("SPIFFWORKFLOW_BACKEND_RUN_API_ENDPOINTS", True):
        start_apscheduler_if_appropriate(app)
        print("Background scheduler started")
    elif app.config.get("SPIFFWORKFLOW_BACKEND_RUN_BACKGROUND_SCHEDULER_IN_CREATE_APP", False):
        start_apscheduler_if_appropriate(app)
        print("Background scheduler started (forced by config)")
